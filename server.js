...
app.get('/proxy', async (req, res) => {
  try {
    ...
    // For localhost URLs, use simpler headers to avoid connection issues
    ...
    if (contentType.includes('text/html')) {
      ...
      const syncScript = `\n<script>(function(){try{function getScrollEl(){try{var doc=document;if(doc.scrollingElement&&doc.scrollingElement.scrollHeight>doc.scrollingElement.clientHeight){return doc.scrollingElement;}var list=Array.prototype.slice.call(doc.querySelectorAll('body, body *'));var cand=list.filter(function(el){try{var st=getComputedStyle(el);var oy=st.overflowY;return (oy==='auto'||oy==='scroll')&&(el.scrollHeight-1>el.clientHeight+1);}catch(_){return false;}});cand.sort(function(a,b){return (b.scrollHeight-b.clientHeight)-(a.scrollHeight-a.clientHeight);});return cand[0]||doc.documentElement||doc.body;}catch(_){return document.documentElement||document.body;}}var SCROLL_EL=getScrollEl();window.addEventListener('message',function(e){var d=e&&e.data||{};if(d.type==='SYNC_SCROLL_BY'){try{if(SCROLL_EL&&SCROLL_EL.scrollTop!=null){SCROLL_EL.scrollTop += (d.dy||0);}else{window.scrollBy(0,d.dy||0);}}catch(_){}}else if(d.type==='SYNC_SCROLL_TO'){try{if(SCROLL_EL&&SCROLL_EL.scrollTop!=null){SCROLL_EL.scrollTop = d.y||0; SCROLL_EL.scrollLeft = d.x||0;}else{window.scrollTo(d.x||0,d.y||0);}}catch(_){}}else if(d.type==='SCROLL_TO_SELECTOR'){try{var sel=d.selector||'';var off=d.offset||0;var el=sel?document.querySelector(sel):null; if(el){var r=el.getBoundingClientRect();var baseY=(SCROLL_EL&&SCROLL_EL.scrollTop!=null?SCROLL_EL.scrollTop:(window.scrollY||0)); var y=baseY + r.top + off; if(SCROLL_EL&&SCROLL_EL.scrollTop!=null){SCROLL_EL.scrollTop = y;} else {window.scrollTo(0,y);} }}catch(_){}}},{passive:true});var raf;function emit(){try{var x=SCROLL_EL&&SCROLL_EL.scrollLeft!=null?SCROLL_EL.scrollLeft:(window.scrollX||0);var y=SCROLL_EL&&SCROLL_EL.scrollTop!=null?SCROLL_EL.scrollTop:(window.scrollY||0);parent&&parent.postMessage({type:'SCROLL_POS',x:x,y:y},'*');}catch(_){}}function q(){if(raf)cancelAnimationFrame(raf);raf=requestAnimationFrame(emit);}window.addEventListener('scroll',q,{passive:true,capture:true});if(SCROLL_EL&&SCROLL_EL!==document&&SCROLL_EL!==document.body){SCROLL_EL.addEventListener('scroll',q,{passive:true,capture:true});}window.addEventListener('wheel',function(e){try{parent&&parent.postMessage({type:'SCROLL_BY',dy:(e&&e.deltaY)||0},'*');}catch(_){ }},{passive:true,capture:true});window.addEventListener('resize',function(){SCROLL_EL=getScrollEl();},{passive:true});}catch(_){}})();</script>`;
